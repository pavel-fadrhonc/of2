...
{
    public class HermiteCurveIndex
    {
        public Vector3[] linearPoints;
        HermiteCurve curve;


        public int ControlPointCount => curve.ControlPointCount;


        public HermiteCurveIndex(HermiteCurve curve)
        {
            this.curve = curve;
            ReIndex();
        }
        ...
        public void ReIndex()
        {
            ...
            while (t <= 1f)
            {
                var current = curve.GetNonUniformPoint(t);
                while ((current - start).sqrMagnitude <= linearDistanceStep2)
                {
                    t += searchStepSize;
                    current = curve.GetNonUniformPoint(t);
                }
                start = current;
        ...
        {
            var sections = linearPoints.Length - (curve.closed ? 0 : 3);
            ...
            //TODO: decide how to actually interpolate
